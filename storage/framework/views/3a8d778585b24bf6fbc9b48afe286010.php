...
<head>
    ...
    <title><?php echo e($title ?? 'Dashboard'); ?> — Supervisi Guru</title>
...
<div class="min-h-screen lg:flex">
    ...
    <aside class="hidden lg:flex flex-col w-60 shrink-0 bg-white min-h-screen sticky top-0 h-screen border-r border-slate-100">
        <div class="px-5 py-6 flex items-center gap-2.5">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($schoolLogo): ?>
                <img src="<?php echo e($schoolLogo); ?>" alt="Logo sekolah" class="w-9 h-9 rounded-xl object-contain bg-white ring-1 ring-slate-200 shrink-0">
            <?php else: ?>
        ...
        <nav class="flex-1 overflow-y-auto px-4 pb-3 space-y-1">
            <?php echo $navLink('Dashboard', '▦', route('dashboard'), request()->routeIs('dashboard')); ?>

            <?php echo $navLink('Jadwal Supervisi', '◷', route('supervisions.index'), request()->routeIs('supervisions.*'), $notifItems->count() ?: null); ?>

            <?php echo $navLink($canReview ? 'Dokumen' : 'Dokumen Saya', '▤', route('documents.index'), request()->routeIs('documents.*')); ?>

            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isSupervisorArea): ?>
                <?php echo $navLink('Pra Observasi', '◎', route('pre-observations.index'), request()->routeIs('pre-observations.*')); ?>

                <?php echo $navLink('Pemantauan', '◉', route('pemantauan.index'), request()->routeIs('pemantauan.*')); ?>

                <?php echo $navLink('Pasca Supervisi', '⧉', route('post-supervisions.index'), request()->routeIs('post-supervisions.*')); ?>

            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isAdmin): ?>
                <?php echo $navLink('Pengguna', '◍', route('users.index'), request()->routeIs('users.*')); ?>

                <?php echo $navLink('Mata Pelajaran', '≡', route('subjects.index'), request()->routeIs('subjects.*')); ?>

                <?php echo $navLink('Periode', '◐', route('periods.index'), request()->routeIs('periods.*')); ?>

                <?php echo $navLink('Pengaturan', '⚙', route('settings.index'), request()->routeIs('settings.*')); ?>
        ...
        <div class="px-4 pb-3">
            <div class="rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-500 p-4 text-white text-center">
                ...
                <p class="font-bold text-sm mt-0.5"><?php echo e($period ? $period->tahun_ajaran : '—'); ?></p>
                <p class="text-xs opacity-80"><?php echo e($period ? $period->semester : 'Belum dipilih'); ?></p>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isAdmin): ?>
                    <a href="<?php echo e(route('periods.index')); ?>" class="inline-block mt-2.5 text-xs font-semibold bg-white text-indigo-600 rounded-lg px-4 py-1.5">Kelola</a>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            </div>
            <div class="flex items-center gap-2.5 mt-3 px-1">
                <div class="w-9 h-9 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-sm shrink-0"><?php echo e(strtoupper(substr($user->name, 0, 1))); ?></div>
                <div class="min-w-0 flex-1">
                    <p class="text-[13px] font-semibold truncate"><?php echo e($user->name); ?></p>
                    <p class="text-[11px] text-slate-400 truncate"><?php echo e(implode(', ', $user->role_labels)); ?></p>
                </div>
    ...
    <div class="lg:hidden bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-2.5 sticky top-0 z-30">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($schoolLogo): ?>
            <img src="<?php echo e($schoolLogo); ?>" alt="Logo sekolah" class="w-8 h-8 rounded-lg object-contain bg-white ring-1 ring-slate-200 shrink-0">
        <?php else: ?>
        ...
        <span class="text-xs font-medium bg-slate-100 rounded-lg px-2.5 py-1.5 text-slate-500">🔔<?php echo e($notifItems->count() ? ' '.$notifItems->count() : ''); ?></span>
    </div>
    ...
    <div id="mobile-drawer" class="hidden fixed inset-0 z-40 lg:hidden">
        ...
        <aside class="absolute inset-y-0 left-0 w-64 bg-white flex flex-col shadow-2xl overflow-y-auto">
            ...
            <nav class="flex-1 px-4 pb-3 space-y-1">
                ...
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                <?php echo $navLink('Profil Saya', 'ⓘ', route('profile.edit'), request()->routeIs('profile.*')); ?>
    ...
    <div class="flex-1 min-w-0 px-4 md:px-8 py-5 md:py-7 max-w-[1200px] mx-auto w-full">
        <div class="flex items-center gap-3 mb-5">
            <h1 class="text-xl md:text-2xl font-extrabold flex-1"><?php echo e($title ?? 'Dashboard'); ?></h1>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($period): ?>
                <span class="hidden sm:inline-block text-xs font-medium bg-white rounded-lg px-3 py-2 shadow-sm text-slate-500"><?php echo e($period->start_date?->format('d-m-Y')); ?></span>
                <span class="hidden sm:inline-block text-xs font-medium bg-white rounded-lg px-3 py-2 shadow-sm text-slate-500"><?php echo e($period->end_date?->format('d-m-Y')); ?></span>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <span class="text-xs font-medium bg-white rounded-lg px-3 py-2 shadow-sm text-slate-500" title="Jadwal mendatang">🔔<?php echo e($notifItems->count() ? ' '.$notifItems->count() : ''); ?></span>
        </div>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('success') || session('error')): ?>
            <div class="mb-4 px-4 py-3 rounded-2xl text-sm bg-white shadow-sm border-l-4 <?php echo e(session('success') ? 'border-emerald-400 text-emerald-700' : 'border-rose-400 text-rose-700'); ?>">
...
<?php /**PATH D:\project\supervisi_guru laravel\resources\views/layouts/app.blade.php ENDPATH**/ ?>